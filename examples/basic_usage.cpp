...
int main() {
    ...
    std::cout << "  Dimensions: " << params.dimensions[0] << " x "
              << params.dimensions[1] << " x " << params.dimensions[2] << std::endl;
    std::cout << "  Spacing: (" << params.spacing.x << ", "
              << params.spacing.y << ", " << params.spacing.z << ")" << std::endl;
    std::cout << "  Range: [" << params.min_bound.x << ", " << params.max_bound.x << "] x ["
              << params.min_bound.y << ", " << params.max_bound.y << "] x ["
              << params.min_bound.z << ", " << params.max_bound.z << "]" << std::endl;
    std::cout << std::endl;
    ...
    if (err == ErrorCode::Success && result.valid) {
        ...
        std::cout << "  Gradient vector: (" << result.data.gradient_x << ", "
    ...
    for (size_t i = 0; i < num_queries; ++i) {
        Point3D point(
            params.min_bound.x + (params.max_bound.x - params.min_bound.x) * (rand() / double(RAND_MAX)),
    ...
    if (err == ErrorCode::Success) {
        ...
        for (size_t i = 0; i < std::min(size_t(5), num_queries); ++i) {
            ...
            if (res.valid) {
                std::cout << "  Point " << i + 1 << ": (" << std::fixed << std::setprecision(2)
                          << point.x << ", " << point.y << ", " << point.z << ") -> B = "
                          << std::setprecision(4) << res.data.field_strength << std::endl;
            }
        }
    } else {
