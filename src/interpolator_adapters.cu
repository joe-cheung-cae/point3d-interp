...
CPUStructuredInterpolatorAdapter::CPUStructuredInterpolatorAdapter(std::unique_ptr<RegularGrid3D> grid,
...
size_t CPUStructuredInterpolatorAdapter::getDataCount() const { return grid_->getDataCount(); }
...
GridParams CPUStructuredInterpolatorAdapter::getGridParams() const { return grid_->getParams(); }

std::vector<Point3D> CPUStructuredInterpolatorAdapter::getCoordinates() const { return grid_->getCoordinates(); }

std::vector<MagneticFieldData> CPUStructuredInterpolatorAdapter::getFieldData() const { return grid_->getFieldData(); }
...
CPUUnstructuredInterpolatorAdapter::CPUUnstructuredInterpolatorAdapter(
    std::unique_ptr<UnstructuredInterpolator> interpolator, InterpolationMethod method,
    ExtrapolationMethod extrapolation)
    : unstructured_interpolator_(std::move(interpolator)), method_(method), extrapolation_(extrapolation) {}
...
size_t CPUUnstructuredInterpolatorAdapter::getDataCount() const { return unstructured_interpolator_->getDataCount(); }
...
GPUStructuredInterpolatorAdapter::GPUStructuredInterpolatorAdapter(std::unique_ptr<RegularGrid3D> grid,
                                                                   InterpolationMethod            method,
                                                                   ExtrapolationMethod            extrapolation)
    : grid_(std::move(grid)),
      cpu_interpolator_(std::make_unique<CPUInterpolator>(*grid_, extrapolation)),
...
size_t GPUStructuredInterpolatorAdapter::getDataCount() const { return grid_->getDataCount(); }
...
GridParams GPUStructuredInterpolatorAdapter::getGridParams() const { return grid_->getParams(); }

std::vector<Point3D> GPUStructuredInterpolatorAdapter::getCoordinates() const { return grid_->getCoordinates(); }

std::vector<MagneticFieldData> GPUStructuredInterpolatorAdapter::getFieldData() const { return grid_->getFieldData(); }
...
std::vector<InterpolationResult> GPUUnstructuredInterpolatorAdapter::queryBatch(
    ...
    if (!spatial_grid_.cell_offsets.empty() && !spatial_grid_.cell_points.empty()) {
        ...
        p3d::cuda::IDWSpatialGridKernel<<<num_blocks, num_threads>>>(
            d_query_points.getDevicePtr(), d_points_.getDevicePtr(), d_field_data_.getDevicePtr(), data_count,
            d_cell_offsets_.getDevicePtr(), d_cell_points_.getDevicePtr(), spatial_grid_.origin,
            spatial_grid_.cell_size, spatial_grid_.dimensions[0], spatial_grid_.dimensions[1],
            spatial_grid_.dimensions[2], power, extrapolation_method, min_bound, max_bound, d_results.getDevicePtr(),
            points.size());
    } else {
        // Fallback to brute force kernel
        p3d::cuda::IDWInterpolationKernel<<<num_blocks, num_threads>>>(
            d_query_points.getDevicePtr(), d_points_.getDevicePtr(), d_field_data_.getDevicePtr(), data_count, power,
            extrapolation_method, min_bound, max_bound, d_results.getDevicePtr(), points.size());
    }
...
size_t GPUUnstructuredInterpolatorAdapter::getDataCount() const { return unstructured_interpolator_->getDataCount(); }
